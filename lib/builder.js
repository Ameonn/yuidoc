...
YUI.add('doc-builder', function(Y) {
    ...
    Y.DocBuilder.prototype = {
        ...
        makeDirs: function(cb) {
            var self = this;
            var dirs = ['classes', 'elements', 'modules', 'files', 'interfaces'];
        ...
        */
         ...
         */
        _mergeCounter: null,
        _mergeMethods: function(type, info, hasItems, items) {
          var self = this;
            this.data[type + 'items'].forEach(function(v) {
                //console.error(v[type], '==', info.extends);
                if (hasItems[v[type]]) {
                    if (!v.static) {
                        var q,
                        ...
                        if (override === false) {
                            ...
                            //console.error('Merging extends from', v[type], 'onto', info.name);
                            q = Y.merge({}, v);
                            q.extended_from = v[type];
                            items.push(q);
                        } else {
                            //This method was extended from the parent and overwritten in this class
                            q = Y.merge({}, v);
                            q = self.augmentData(q);
                            q.overwritter = info.name;
        ...
        mergeExtends: function(info, items, first, type) {
            ...
            if (info.extends || info.uses) {
                ...
                items = this._mergeMethods(type, info, hasItems, items);
        ...
        mergeImplements: function(info, items, first) {
            ...
            if (info.implements) {
              info.implements.forEach(function (minterface) {
                ...
                items = self._mergeMethods('interface', info, hasItems, items);
        ...
        renderInterface: function(cb, data, layout) {
            ...
            Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function(err, opts) {
                ...
                var interfaceItems = [];
                self.data.interfaceitems.forEach(function(interfaceItem) {
                    if (interfaceItem.interface === data.name) {
                        interfaceItems.push(interfaceItem);
                    }
                });
        ...
        writeComponents: function(type, cb, layout) {
            ...
            Y.each(self.data[type], function(v) {
                ...
                self[render](stack.add(function(html, view) {
                    stack.html.push(html);
                    stack.view.push(view);
                    if (self.options.dumpview) {
                        Y.Files.writeFile(
        ...
        renderAPIMeta: function(cb) {
            ...
            ['classes', 'modules', 'elements', 'interfaces'].forEach(function(id) {
