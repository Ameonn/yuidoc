        ...
        DIGESTERS = {
            ...
            'submodule': function(tagname, value) {
                ...
                var host = this.modules[value],
                    ...
                    minterface = this.get(CURRENT_INTERFACE),
                ...
                if (clazz && this.classes[clazz]) {
                    ...
                    //}
                } else if (minterface && this.interfaces[minterface]) {
                  this.interfaces[minterface].submodule = value;
                }
                return host;
            },
            ...
            'class': function(tagname, value, target, block) {
                ...
                block.forEach(function(def) {
                    if (def.tag === 'namespace') {
                        ...
                        var name = trim(def.value) + '.' +
                            value;
                        if (value.indexOf(trim(def.value) +
                                '.') === -1) {
                            value = name;
            ...
            //A key bock type for declaring minterface
            'interface': function(tagname, value, target, block) {
                var namespace, fullname, host, parent;
                ...
                this.set(CURRENT_INTERFACE, value);

                fullname = this.get(CURRENT_INTERFACE);
                host = this.interfaces[fullname];
                parent = this.get(CURRENT_MODULE);
            ...
            'property': function(tagname, value, target, block) {
                ...
                if (target.type && target.type.toLowerCase() ===
                    'object') {
            ...
            'extensionfor': function(tagname, value) {
                ...
                } else if (this.interfaces[this.get(CURRENT_INTERFACE)]) {
                  this.interfaces[this.get(CURRENT_INTERFACE)].extension_for.push(value);
                }
            ...
            'namespace': function(tagname, value) {
                ...
                var currentClass = this.get(CURRENT_CLASS);
                var currentInterface = this.get(CURRENT_INTERFACE);
                var container = currentClass ? 'classes' : 'interfaces';
                var setter = currentClass ? CURRENT_CLASS : CURRENT_INTERFACE;
                mod = currentClass || currentInterface;
                if (mod) {
            ...
            'implements': function(tagname, value) {
              var currentObject = this.get(CURRENT_CLASS) || this.get(CURRENT_INTERFACE);
              var type = this.get(CURRENT_CLASS) ? CURRENT_CLASS : CURRENT_INTERFACE;
              var container = type === CURRENT_CLASS ? 'classes' : 'interfaces';
    ...
    DocParser.ATTRS = {
        ...
        currentmodule: {
            setter: function(val) {
                ...
                var currentClass = this.classes[this.get(CURRENT_CLASS)];
                var currentInterface = this.interfaces[this.get(CURRENT_INTERFACE)];
                var obj = currentClass || currentInterface;
                var container = currentClass ? 'classes' : 'interfaces';
                if (obj) {
        ...
        currentnamespace: {
            setter: function(val) {
                ...
                this.set(LAST_NAMESPACE, this.get(
                    CURRENT_NAMESPACE));
                return val;
            }
        },
        ...
        currentinterface: {
            setter: function(val) {
                ...
                this.set('lastinterface', this.get(
                    CURRENT_INTERFACE));
                val = trim(val);
                var name = val,
                    ns, minterface;
        ...
        **/
        ...
        processblock: function(block) {
                ...
                if (tag && tag.tag) {
                    if (!(name in this.knowntags)) {
                        if (name in CORRECTIONS) {
                            this.warnings.push({
                                message: 'replacing incorrect tag: ' +
                                    name +
                                    ' with ' +
                                    CORRECTIONS[
                                        name
                    ...
                    if (digestname in this.digesters) {
                        digester = this.digesters[
                            digestname];
                        if (Lang.isString(digester)) {
                            digester = this.digesters[
                                digester];
                        }
                        ret = digester.call(this,
                            name, value, target,
                            block);
                        host = host || ret;
                    } else {
            ...
            if (host) {
                ...
                if (host.attributes && target.attributes) {
                    host.attributes.push.apply(host.attributes,
                        target.attributes);
                }
            } else if (target.attributes) {
                host = this.elements[this.get(
                    CURRENT_ELEMENT)];
                if (host) {
                    if (target.deprecated) {
                        target.attributes.forEach(function(
                            a) {
            ...
            } else if (this.get(CURRENT_CLASS)) {
            ...
            } else if (this.get(CURRENT_INTERFACE)) {
                this.interfaceitems.push(target);
                target.interface = this.get(CURRENT_INTERFACE);
                target.module = this.get(CURRENT_MODULE);

                var submodule = this.get(CURRENT_SUBMODULE);
                if (submodule) {
                  target.submodule = submodule;
                }

                var namespace = this.get(CURRENT_NAMESPACE);
                if (namespace) {
        ...
        transform: function(commentmap) {
            var self = this,
                ...
                interfaces = self.interfaces = {},
                interfaceitems = self.interfaceitems = [],
            ...
            Y.each(modules, function(module, name) {
                if (module.file) {
                    files[module.file].modules[name] =
                        1;
                }
                if (module.is_submodule) {
                    modules[module.module].submodules[
                        name] = 1;
                }
                //Clean up processors
            ...
            Y.each(elements, function(el, name) {
                if (el.module) {
                    modules[el.module].elements[
                        name] = 1;
                }

                if (el.submodule) {
                    modules[el.submodule].elements[
                        name] = 1;
                    if (!modules[el.submodule].description) {
                        modules[el.submodule].description =
                            el.description;
                    }
                }
            });
        ...
        parse: function(filemap, dirmap) {
            filemap = filemap || this.get('filemap');
            dirmap = dirmap || this.get('dirmap');
            return this.transform(this.extract(filemap,
                dirmap));
        }
    });
